...
**Rule:** every number is real StatCan 2021 Census data. No value was fabricated. Where a value is unavailable (suppressed or not published), it is `null` and noted below.

All Statistics Canada data here is released under the **Statistics Canada Open Licence** (https://www.statcan.gc.ca/en/reference/licence), which permits reproduction and redistribution with attribution.

---

## File 1 — Geometry: `tide/tide-web/public/peel-fsa.geojson`
...
No GitHub mirror fallback was needed — the StatCan shapefile downloaded and reprojected in well under the 15-minute budget.

---

## File 2 — Attributes: `tide/tide-web/lib/peel-fsa-raw.json`
...
**Extraction:** used the `Geo_starting_row` index to locate each Peel FSA's block (DGUID prefix `2021A0011<FSA>`, GEO_LEVEL = Forward sortation area), sliced those line ranges from the 645 MB CSV, and parsed with Python's `csv` module. Values taken from columns `C1_COUNT_TOTAL` (counts) and `C10_RATE_TOTAL` (published rates).
...
| `electricHeatPct` | — | **Not published** in 98-401-X2021013 at the FSA level (no heating-fuel characteristic exists in this profile). Set to `null` for every FSA. |
...
**No "burden score" or "intervention" was computed** — this file is raw cited facts only.
...
Sum of population across all 35 FSAs = **1,451,271**, consistent with Peel Region's published 2021 Census population (~1.45 M) — a sanity check that the FSA set is complete and correctly extracted.
...
| FSA | Status | Reason |
|---|---|---|
| **L5P** | Not in any 2021 file | Code does not exist in the 2021 Census FSA boundary file or profile (former Pearson-airport-area FSA; not a residential FSA in 2021). Skipped per instructions — not invented. Expected count was 36, actual is **35**. |
...
**Load-bearing coverage:** 32 of 35 FSAs have non-null `population`, `medianHouseholdIncome`, and `renterPct`. The 3 nulls (L4V, L5S, L5T) are genuine StatCan suppressions on FSAs with <30 residents, not extraction failures.
