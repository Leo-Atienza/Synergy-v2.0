...
> **Status:** these are two **prototypes**, not the official submission scaffold (that gets stood up at `/hackathon:scaffold` after the May 24 kickoff). The engine is proven; the packaging is deliberately throwaway. Read [`valley/spike/VIABILITY.md`](../spike/VIABILITY.md) for the verdict.
...
## Part A — the engine + CLI (`valley/spike`)
...
## Part B — the demo UI (`valley/tide-web`)
...
```bash
cd valley/spike && npm test && npm run typecheck   # engine: 7 tests + strict types
...
1. **The engine is duplicated.** `valley/spike/src/{optimizer,rates,carbon,savings,types}.ts` and `valley/tide-web/lib/*` are **parallel copies**. Change the optimizer in one and the other silently drifts. This is a deliberate hackathon shortcut; the real scaffold should make it one shared module. Until then: **change both.**
...
- [`valley/spike/README.md`](../spike/README.md) — the spike's own run notes
- [`valley/spike/VIABILITY.md`](../spike/VIABILITY.md) — what's proven, what's still open
