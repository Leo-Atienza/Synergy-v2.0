...
> - **FSA** is the first three characters of a postal code (e.g., "L4X"). Roughly a neighbourhood. Our map uses Peel's 35 FSAs.
> - **Sub-metered** means an apartment building with one main meter that a third-party company divides among units. The **tenant can't pick their own price plan.** The building owner does.
> - **Load-shifting** means running an appliance at a cheaper time of day to save money.

---

## The whole idea in one minute

Ontario has a super-cheap overnight electricity rate. It's **3.9¢ at 3 a.m. versus 39.1¢ at dinner, ten times cheaper.** It exists to reward people who move their power use to off-hours.

Here's the catch. The people who most need a cheaper bill are **low-income renters**, and they usually can't take it.
- They can't be awake at 3 a.m. to run the laundry, so switching to this plan often makes their bill **go up**, not down.
- Many of them **aren't even allowed to choose it.** In a lot of apartments the landlord picks the building's rate plan, not the tenant.
...
2. **One burden score out.** Valley blends those into a single **0–100 energy-burden score** per neighbourhood (more renters plus lower income means higher burden) and colours the map from teal (okay) to red (worst). Malton (L4X) comes out the reddest.
3. **Click a neighbourhood and it tells you the right fix.** This triage is the clever part:
   - **Lots of apartments means "Policy fix."** Mostly sub-metered, so the landlord picks the plan and the renter is locked out of ULO. A plug can't help here.
   - **Mostly individually-metered renters means "Valley reaches this."** These renters *can* choose ULO but get punished by it. They're exactly who the plug helps.
...
Ontario built the cheap overnight rate to **reward flexibility**. But the people who most need a cheaper bill are the **least able to flex**, and that creates the two problems Valley is built around:
...
**Realistic (what we'll say on camera, and it's honest):**
...
- Grid-meaningful scale (50–100 MW) means 50,000–100,000 homes. That's a multi-year build, which is why it's the vision, not the pitch.
...
- **Sell the targeting map to utilities and energy-poverty programs.** They already spend on outreach; the map makes that spend efficient (device vs policy vs retrofit, per neighbourhood). Recurring, no hardware, high margin. **This is the real business.**
...
3. **Map as a product:** sell the triage layer to utilities, with no hardware needed to scale it.
...
| *"Hasn't load-shifting been done?"* | "Yes, separately (Emporia, Optiwatt). Our slice is the renter plus the legal lockout plus the triage map." |
...
- **Sub-metered tenants can't choose their plan:** OEB says "that decision can only be made for the building as a whole by the master consumer" (landlord/property manager). Source: OEB "Understanding electricity unit" (USMP).
